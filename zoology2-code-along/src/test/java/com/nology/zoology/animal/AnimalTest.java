...
//        animal = new Animal(ANIMAL_ID, ANIMAL_NAME, ANIMAL_AGE) {
//            @Override
...
//    }
    @Test
    @DisplayName("shouldt work with bad for name")
    void setingBadAge() {
        try {
            Crocodile zeroCroc = new Crocodile(1, "josh", 0);
            Crocodile onehundredCroc = new Crocodile(1, "josh", 100);
    ...
    @Test
    @DisplayName("shouldt work with null for name")
    void setingNullName(){
        Crocodile nullCroc = null;
//        Crocodile emptyCroc;
        try {
        nullCroc = new Crocodile(1,null,33);
        fail("name cant be null or empty");
//        emptyCroc = new Crocodile(1,"",33);
//        assertNull(emptyCroc.getName());
//        assertNull(nullCroc.getName());
        } catch (IllegalArgumentException e) {
        }
    }
}
